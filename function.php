...
function writepartnerprefs($id){
  if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($result) {
      ...
      echo "<script> window.location=\"userprofile.php?id=$id\"</script>";
...
if(mysqli_num_rows($result)>=1){
  ...
  $sql = "INSERT 
        INTO
           customer
           (cust_id, email, age, sex, religion, division, district,  country, maritalstatus,  education, firstname, lastname,phonenumber, body_type, physical_status, drink, mothertounge, colour, weight, height, blood_group, diet, smoke,   dateofbirth, occupation,  annual_income, fathers_occupation, mothers_occupation, no_bro, no_sis, aboutme, profilecreationdate, image  ) 
        VALUES
           ('$id','$email', '$age', '$sex', '$religion', '$division', '$district',  '$country', '$maritalstatus',  '$education', '$fname', '$lname','$phonenumber', '$bodytype', '$physicalstatus', '$drink', '$mothertounge', '$colour', '$weight', '$height', '$bloodgroup', '$diet', '$smoke', '$dob', '$occupation',  '$income', '$fatheroccupation', '$motheroccupation', '$bros', '$sis', '$aboutme', CURDATE(),'image')
      ";
  if (mysqli_query($conn,$sql)) {
...
$sql="INSERT INTO photos (id, cust_id, pic1, pic2, pic3, pic4) VALUES ('', '$id', '$pic1' ,'$pic2', '$pic3','$pic4')";
    ...
    mysqlexec($sql);
  $sql1="UPDATE customer SET image = '$pic1' WHERE cust_id=$id";  
  mysqlexec($sql1);  
} else {
